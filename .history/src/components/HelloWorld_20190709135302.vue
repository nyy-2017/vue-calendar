<template>
  <div class="hello">
     <h1>日历测试</h1>
     <div>当前选中的年份/月份:{{curYearMonths}}</div>
     <div>当前年月未被选中的日期:{{unSelectDays}}</div>
      <input type="text" @click="openMultiByDrop($event)" v-model="calendar5.display" >
      <transition name="fade">
        <div class="calendar-dropdown" :style="{'left':calendar5.left+'px','top':calendar5.top+'px'}" v-if="calendar5.show">
            <calendar 
                :zero="calendar5.zero" 
                :disabled="calendar5.disabled" 
                :lunar="calendar5.lunar" 
                :value="calendar5.value" 
                :multi="calendar5.multi" 
                @currentYearMon="calendar5.currentYearMon"
                @select="calendar5.select"></calendar>
        </div>
      </transition>
  </div>
</template>

<script>
import calendar from '@/components/calendar/calendar.vue'
 
export default {
    name: 'app',
    components: {
        calendar
    },
    data(){
        return {
            unSelectDays: '',
            curYearMonths:'',
            // 多选
            calendar5:{
                display:"2019/7/8,2019/7/9",
                multi:true,
                show:false,
                zero:true,
                value:[[2019,7,8],[2019,7,9]], //默认日期
                disabled:[[2017,12,24],[2017,12,25]], //默认日期
                lunar:true, //显示农历
                select:(value)=>{

                    let displayValue=[]
                    value.forEach(v=>{
                        displayValue.push(v[0]+"/"+(v[1])+"/"+v[2])
                    })
                    console.log(displayValue);
                    this.calendar5.display=displayValue.join(",");
                    // this.calendar5.show=false;
                    this.calendar5.value=value;
                    
                },
                currentYearMon:(year,mon)=> {
                     
                    let currSelectDays = this.calendar5.value.filter(v => {
                        return year===v[0]&&mon===v[1]
                    })
                    this.chooseReverse(year, mon, currSelectDays)
                   
                }
            },
        }
    },
    methods:{
        openMultiByDrop(e){
            this.calendar5.show=true;
            this.calendar5.left=e.target.offsetLeft;
            this.calendar5.top=e.target.offsetTop+50;
            e.stopPropagation();
            window.setTimeout(()=>{
                document.addEventListener("click",(e)=>{
                    this.calendar5.show=false;
                    document.removeEventListener("click",()=>{},false);
                },false);
            },1000)
        },
        //当月反选
        chooseReverse(year, mon, currSelectDays){
            // 获取当前月天数
            let daysCount = new Date(year, mon, 0).getDate() 
            var fullMonth = []; 
            for(var i= 1; i <= daysCount; i++) {
                let _date = [year, mon, i]
                fullMonth.push(_date)
            }
          
            // 从当前月去掉当前选中状态的日期
            let reverse = []
            let _fullMonth = []

            fullMonth.map(v =>  _fullMonth.push(v[2]))
            currSelectDays.map(v => {
                _fullMonth.map(full => {
                    if (v[2] === full) {
                       let index = _fullMonth.indexOf(v[2])
                       _fullMonth.splice(index, 1)
                    } 
                })
            })
            _fullMonth.map(v => reverse.push([year, mon, v]) )
            if (reverse.length == 0){
                this.unSelectDays= '当月全部都被选中'
            } else {
                let unSelect = [];
                reverse.map(v => unSelect.push(v[2]))
                // 当前年月未被选中日期
                this.unSelectDays = + reverse[0][0] + '-' + reverse[0][1]+' (' + unSelect.join(',') + ')';
                //当前年月
                curYearMonths = reverse[0][0] + '年' + reverse[0][1] + '月';
                console.log(' this.unSelectDays', this.unSelectDays)
            }
             
        }
        
    },
    created() {
        let year = new Date().getFullYear()
        let mon = new Date().getMonth() + 1
        let day = new Date().getDate()
        this.calendar5.value = [[year, mon, day]]
        this.calendar5.display = + year + '/' + mon + '/' + day
        this.calendar5.currentYearMon(year, mon, this.calendar5.value)
    }

}
</script>

<style>
input{
    box-sizing: border-box;
    width:100%;
    width: 250px;
    margin-top:20px;
    border-radius: 2px;
    border:1px solid #dedede;
    padding:10px;
    font-size: 16px;
    background:url(data:image/svg+xml;base64,) no-repeat 200px 10px;
    /* padding-left: 36px; */
    color:#666;
}

/*transition*/
.fade-enter-active,
.fade-leave-active {
    transition: all .5s ease-in-out;
}
.fade-enter,.fade-leave-active{
    opacity: 0;
    transform: translateY(-10px);

}

/*下拉框*/
.calendar-dropdown{
    background: #fff;
    position: absolute;
    left:0;
    top:0;
    padding:20px;
    border: 1px solid #eee;
    border-radius: 2px;
}
.calendar-dropdown:before {
    position: absolute;
    left:30px;
    top: -10px;
    content: "";
    border:5px solid rgba(0, 0, 0, 0);
    border-bottom-color: #DEDEDE;
}
.calendar-dropdown:after {
    position: absolute;
    left:30px;
    top: -9px;
    content: "";
    border:5px solid rgba(0, 0, 0, 0);
    border-bottom-color: #fff;
}

/*弹出框*/
.calendar-dialog{
    position: absolute;
    left:0;
    top:0;
    right:0;
    bottom:0;
}

.calendar-dialog-mask{
    background:rgba(255,255,255,.5);
    width:100%;
    height:100%;
}

.calendar-dialog-body{
    background: #fff;
    position: absolute;
    left:50%;
    top:50%;
    transform: translate(-50%,-50%);
    padding:20px;
    border: 1px solid #eee;
    border-radius: 2px;
}
</style>
